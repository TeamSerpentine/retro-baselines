...
class Board(BaseBoard):
    ...
    def __repr__(self):
        return f"<class Board (np.array(width={self.width}, heigth={self.height}), dtype=np.object)"
    ...
    def _step_snake(self, snake):
        ...
        # If apple is eaten
        if isinstance(new_position_board, Apple):
            self._reward.append(constants.DEFAULT_REWARD_PER_APPLE)
            self._collision_apple(snake)
        else:
    ...
    def _step(self, action: int):
        ...
        for snake in living_snakes:
            snake.direction = constants.GET_ACTION_MEANING[action]
            self._step_snake(snake)

    def _collision_apple(self, snake):
        """
        ...
        to_remove = Apple(snake.next_step_head())
    ...
    def _collision_dead(self, snake):
        ...
        """
        snake.alive = False
        for each in snake.location():
            self.board[each.as_value()] = Ground(each.as_value())

    def _get_obs(self, attribute="text"):
